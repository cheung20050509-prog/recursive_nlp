"""Recursive ITHP with BERT-base-Chinese backbone (CH-SIMSv2)."""

import torch
from torch import nn
from transformers.models.bert.modeling_bert import BertModel, BertPooler, BertPreTrainedModel
...
class ITHP_BertModel(BertPreTrainedModel):
    """Same multimodal + syntax stack as ITHP_DebertaModel, with BertModel encoder."""
    ...
    def __init__(self, config, multimodal_config):
        ...
        backbone_path = getattr(multimodal_config, "model", None) or "bert-base-chinese"
        self.model = BertModel.from_pretrained(backbone_path).to(DEVICE)
        ITHP_args = {
            "X0_dim": TEXT_DIM,
            "X1_dim": ACOUSTIC_DIM,
            "X2_dim": VISUAL_DIM,
...
class ITHP_BertForSequenceClassification(BertPreTrainedModel):
    def __init__(self, config, multimodal_config):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.bert = ITHP_BertModel(config, multimodal_config)
    ...
    def forward(
        ...
        if return_syntax_info:
            pooled_output, IB_total, kl_loss_0, mse_0, kl_loss_1, mse_1, recursive_steps, syntax_loss, syntax_info = bert_outputs
        else:
            pooled_output, IB_total, kl_loss_0, mse_0, kl_loss_1, mse_1, recursive_steps, syntax_loss = bert_outputs
