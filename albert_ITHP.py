"""ALBERT + Recursive ITHP: same multimodal path as ITHP_DebertaModel but with AlbertModel text encoder.

HKT (matalvepu) also uses 768-d ALBERT text; the fusion here remains recursive ITHP, not
the HKT paper's cross-attention stack. See `deberta_ITHP.py` for the DeBERTa parallel.
"""
...
class ITHP_AlbertModel(AlbertPreTrainedModel):
    """ALBERT last hidden state -> ITHP (recursive) + optional HCF + syntax tree compose."""
    ...
    def __init__(self, config, multimodal_config):
        ...
        backbone_path = getattr(multimodal_config, "model", None) or "/root/autodl-tmp/recursive_nlp/albert-base-v2"
        self.model = AlbertModel.from_pretrained(backbone_path)
    ...
    def _compose_single_tree(self, token_states, gold_span_mask=None, collect_trace=False):
        ...
        while current_state.size(0) > 1:
            ...
            target_distribution = self._build_merge_target_distribution(
                current_spans, gold_span_mask, merge_logits.dtype
            )
            ...
            current_spans = (
                current_spans[:selected_merge] + [merged_span] + current_spans[selected_merge + 2 :]
            )
            if collect_trace:
                merge_trace.append(selected_merge)
                merged_node = f"({syntax_nodes[selected_merge]} {syntax_nodes[selected_merge + 1]})"
                syntax_nodes = (
                    syntax_nodes[:selected_merge] + [merged_node] + syntax_nodes[selected_merge + 2 :]
                )
    ...
    def _recursive_compose(self, sequence_output, attention_mask, gold_span_masks=None, collect_trace=False):
        ...
        for sample_idx in range(sequence_output.size(0)):
            ...
            sample_root, merge_trace, syntax_tree, sample_syntax_loss, sample_supervised_steps = (
                self._compose_single_tree(
                    sample_tokens, gold_span_mask=sample_gold_span_mask, collect_trace=collect_trace
                )
            )
        ...
        return (
            torch.stack(syntax_roots, dim=0),
            merge_traces,
            syntax_trees,
            syntax_loss,
        )
    ...
    def forward(
        ...
        syntax_root, merge_traces, syntax_trees, syntax_loss = self._recursive_compose(
            sequence_output, attention_mask, gold_span_masks=syntax_span_masks, collect_trace=return_syntax_info
        )
        ...
        if return_syntax_info:
            syntax_info = {"merge_traces": merge_traces, "syntax_trees": syntax_trees}
            return (
                pooled_output,
                IB_total,
                kl_loss_0,
...
class ITHP_AlbertForBinaryClassification(AlbertPreTrainedModel):
    config_class = AlbertConfig

    def __init__(self, config, multimodal_config, **kwargs):
        super().__init__(config)
        # Same attribute name as DeBERTa binary path so checkpoints / debugging stay parallel.
        self.dberta = ITHP_AlbertModel(config, multimodal_config)
    ...
    def forward(
        ...
        if return_syntax_info:
            ...
            pooled_output, IB_total, kl_loss_0, mse_0, kl_loss_1, mse_1, recursive_steps, syntax_loss = ab_outputs
